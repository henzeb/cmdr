...
cmdr::help::show() {
    ...
    local BOLD='' CYAN='' YELLOW='' GREEN='' GRAY='' RESET=''
    if [[ -t 1 ]]; then
        BOLD=$'\e[1m'
        CYAN=$'\e[36m'
        YELLOW=$'\e[33m'
    ...
    if [[ -n "$filter" ]]; then
        ...
        if [[ -n "$_synopsis" ]]; then
            printf "    %scmdr%s %s %s\n" "$BOLD" "$RESET" "${filter//_/ }" "$_synopsis"
        else
            printf "    %scmdr%s %s <command> [arguments]\n" "$BOLD" "$RESET" "${filter//_/ }"
        fi
    else
        ...
        if [[ "$_has_globals" -eq 1 ]]; then
            printf "    %scmdr%s [global options] <command> [arguments]\n" "$BOLD" "$RESET"
        else
            printf "    %scmdr%s <command> [arguments]\n" "$BOLD" "$RESET"
        fi
    fi
    echo ""
    ...
    if [[ -z "$filter" ]]; then
        _cmdr_help_global_options_section "$BOLD" "$CYAN" "$GREEN" "$GRAY" "$RESET"
    fi
    ...
    for module in "${_display_order[@]}"; do
        ...
        if [[ -z "$filter" ]]; then
            ...
            if [[ "$_group" != "$_last_group" ]]; then
                ...
                case "$_group" in
                    general|global) : ;;
                    vendor)  printf "  %s%svendor modules%s\n"  "$YELLOW" "$BOLD" "$RESET" ;;
                    user)    printf "  %s%suser modules%s\n"    "$YELLOW" "$BOLD" "$RESET" ;;
                    project) printf "  %s%scurrent project%s\n" "$YELLOW" "$BOLD" "$RESET" ;;
        ...
        if [[ -z "$module" ]]; then
            ...
            local mhint=""
            [[ -n "${MODULE_REVERSE[$module]+_}" ]] && mhint="${GRAY} (${MODULE_REVERSE[$module]})${RESET}"
            printf "    %s%s%s%s\n" "$BOLD" "$(cmdr::help::display_name "$module")" "$RESET" "$mhint"
        fi
    ...
    if [[ -n "$filter" ]] && declare -f cmdr::help::args_detail > /dev/null 2>&1; then
        cmdr::help::args_detail "$filter" "$col_width" "$BOLD" "$CYAN" "$GREEN" "$GRAY" "$RESET"
    fi
}
