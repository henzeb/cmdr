...
_cmdr_pb_draw() {
    ...
    # Fixed-width fragments (plain text for width calculation)
    ...
    # Bar width = terminal width minus surrounding text and right side
    local bar_w=$(( tw - ${#tag_part} - ${#pct_part} - ${#right_part} ))
    (( bar_w < 4 )) && bar_w=4

    # Truncate right_part if it overflows
    if (( ${#right_part} > tw - ${#tag_part} - ${#pct_part} - 4 )); then
        local max_r=$(( tw - ${#tag_part} - ${#pct_part} - 4 ))
        (( max_r < 0 )) && max_r=0
    ...
    printf '\r\033[2K   \033[1m%s\033[0m  \033[97m%s\033[0m  \033[1m\033[92m%s\033[0m\033[90m%s\033[0m%s\033[90m%s\033[0m' \
        "$CMDR_TAG" "$_CMDR_PB_LABEL" "$filled_bar" "$empty_bar" "$pct_part" "$right_part"
}
